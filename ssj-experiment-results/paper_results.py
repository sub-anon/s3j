...
def draw_single(df: pd.DataFrame, perc: str, legend: bool, xticks: list, yticks=None ,ax=None):
    ...
    if yticks is not None:
        pl = new_df.plot("Time(s)", new_df.columns.get_level_values(0).drop_duplicates()[:-1],
                     ylabel="Latency(s)", ax=ax, legend=False, xticks=xticks, yticks=yticks)
    else:
        pl = new_df.plot("Time(s)", new_df.columns.get_level_values(0).drop_duplicates()[:-1],
                ylabel="Latency(s)", ax=ax, legend=False, xticks=xticks)
...
def draw_single_line_chart(df: pd.DataFrame, perc: str, filename: str, type: str):
    ...
    axes.title.set_text("$\mathregular{S^3J}$\n")
